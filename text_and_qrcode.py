...
def create_qr_code(data, size_mm, dpi):
...
def main():
    ...
    create_qr_code(qr_code_data, qr_code_size_mm, dpi)
    ...
    with open('output.prn', 'wb') as f:
        ...
        # Add the left image
        image_cmd, hex_data = make_image(x_position / MM_TO_DOTS, y_position / MM_TO_DOTS, 'qr_code.png')
        f.write(image_cmd.encode() + hex_data + b"\n")

        # Add the right image
        right_label_x = LABEL_WIDTH_MM + LABEL_SPACING_MM
        image_cmd, hex_data = make_image((right_label_x + x_position / MM_TO_DOTS), y_position / MM_TO_DOTS, 'qr_code.png')
        f.write(image_cmd.encode() + hex_data + b"\n")
